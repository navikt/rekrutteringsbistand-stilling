...
import { ApiError, API_URL, fetchPost, fetchPut } from '../api/api';
...
function* saveStillingsinfo() {
    ...
    try {
        ...
        if (state.stillingsinfoData.stillingsinfoid) {
            response = yield fetchPut(`${API_URL}/rekruttering`, state.stillingsinfoData);
        } else {
            response = yield fetchPost(`${API_URL}/rekruttering`, state.stillingsinfoData);
        }
...
function* updateStillingsinfo() {
    ...
    try {
        ...
        const response = yield fetchPut(`${API_URL}/rekruttering`, state.stillingsinfoData);
