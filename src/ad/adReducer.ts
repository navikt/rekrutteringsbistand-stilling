...
function* createAd() {
    ...
    try {
        ...
        const postUrl = `${API_URL}/rekrutteringsbistand/api/v1/ads?classify=true`;
...
function* saveRekrutteringsbistandStilling(loggPublisering?: boolean) {
    ...
    try {
        ...
        let putUrl = `${API_URL}/rekrutteringsbistandstilling`;
        if (
...
function* deleteAd() {
    ...
    try {
        ...
        const state = yield select();
        const deleteUrl = `${API_URL}/rekrutteringsbistand/api/v1/ads/${state.adData.uuid}`;
